# ---------------------------------------------------------------------------------------------------------------------
# CREATE THE SECURITY GROUP RULES THAT CONTROL WHAT TRAFFIC CAN GO IN AND OUT OF A ZOOKEEPER ENSEMBLE
# ---------------------------------------------------------------------------------------------------------------------
...
resource "aws_security_group_rule" "allow_exhibitor_ui_tcp_inbound" {
  count       = "${length(var.allowed_inbound_cidr_blocks) >= 1 ? 1 : 0}"
  type        = "ingress"
  from_port   = "${var.exhibitor_ui_port}"
  to_port     = "${var.exhibitor_ui_port}"
  protocol    = "tcp"
  cidr_blocks = ["${var.allowed_inbound_cidr_blocks}"]
...
resource "aws_security_group_rule" "allow_exhibitor_ui_tcp_inbound_from_security_group_ids" {
  count                    = "${length(var.allowed_inbound_security_group_ids)}"
  type                     = "ingress"
  from_port                = "${var.exhibitor_ui_port}"
  to_port                  = "${var.exhibitor_ui_port}"
